...
function Checkout() {
	...
	async function handleChangeCep(event) {
		...
		if (cepUnformat.length === 8) {
			return;
		}
		const response = await instanceAxios.get(
			`https://viacep.com.br/ws/${cepUnformat}/json/`
		);
	...
	function handleChangeCpf(event) {
		...
		if (result === messageErro) {
			formik.setFieldError("cpf", messageErro);
		}
	}
	...
	async function handleChangeNumberCard(event) {
		...
		if (numberCardUnformat.length !== 16) {
			return;
		}
	...
	function handleValidateFields(values) {
		...
		if (!values.nome) {
			errors.nome = "campo obrigatorio de ser preenchido";
		} else if (values.nome.length > 30) {
			errors.nome = "insira menos de 30 caracteres";
		}

		if (!values.sobrenome) {
			errors.sobrenome = "campo obrigatorio de ser preenchido";
		} else if (values.sobrenome.length > 40) {
			errors.sobrenome = "insira menos de 40 caracteres";
		}

		if (!values.email) {
			errors.email = "campo obrigatorio de ser preenchido";
		} else if (
		...
		if (!values.cpf) {
			errors.cpf = "campo obrigatorio de ser preenchido";
		} else if (!/^\d{3}\.\d{3}\.\d{3}-\d{2}$/i.test(values.cpf)) {
			errors.cpf = "CPF invalido";
		}

		if (!values.celular) {
			errors.celular = "campo obrigatorio de ser preenchido";
		} else if (!/^\(\d{2}\)\s{1}[9]{1}\d{4}-\d{4}$/i.test(values.celular)) {
			errors.celular = "telefone invalido";
		}

		if (!values.cep) {
			errors.cep = "campo obrigatorio de ser preenchido";
		} else if (!/^\d{5}-\d{3}$/i.test(values.cep)) {
			errors.cep = "CEP invalido";
		}

		if (!values.endereco) {
			errors.endereco = "campo obrigatorio de ser preenchido";
		} else if (values.endereco.length > 50) {
			errors.endereco = "insira menos de 50 caracteres";
		}

		if (!values.numero) {
			errors.numero = "campo obrigatorio de ser preenchido";
		} else if (values.numero.length > 30) {
			errors.numero = "insira menos de 30 caracteres";
		}

		if (!values.bairro) {
			errors.bairro = "campo obrigatorio de ser preenchido";
		} else if (values.bairro.length > 45) {
			errors.bairro = "insira menos de 45 caracteres";
		}

		if (!values.complemento) {
			errors.complemento = "campo obrigatorio de ser preenchido";
		} else if (values.complemento.length > 60) {
			errors.complemento = "insira menos de 60 caracteres";
		}

		if (!values.cidade) {
			errors.cidade = "campo obrigatorio de ser preenchido";
		} else if (values.cidade.length > 30) {
			errors.cidade = "insira menos de 30 caracteres";
		}

		if (!values.estado) {
			errors.estado = "campo obrigatorio de ser preenchido";
		} else if (values.estado.length > 30) {
			errors.estado = "insira menos de 30 caracteres";
		}

		if (!values["num-cartao"]) {
			errors["num-cartao"] = "campo obrigatorio de ser preenchido";
		} else if (
			!/^\d{4}\s{1}\d{4}\s{1}\d{4}\s{1}\d{4}$/i.test(values["num-cartao"])
		) {
			errors["num-cartao"] = "número do cartão invalido";
		}

		if (!values["validade-cartao"]) {
			errors["validade-cartao"] = "campo obrigatorio de ser preenchido";
		} else if (!/^\d{2}\/\d{4}$/i.test(values["validade-cartao"])) {
			errors["validade-cartao"] = "validade do cartão invalido";
		}

		if (!values["cod-seguranca-cartao"]) {
			errors["cod-seguranca-cartao"] = "campo obrigatorio de ser preenchido";
		} else if (!/^\d{1,4}$/i.test(values["cod-seguranca-cartao"])) {
			errors["cod-seguranca-cartao"] =
				"codigo de segurancao do cartão invalido";
		}

		if (!values["nome-cartao"]) {
			errors["nome-cartao"] = "campo obrigatorio de ser preenchido";
		} else if (values["nome-cartao"].length > 60) {
			errors["nome-cartao"] = "insira menos de 60 caracteres";
		}

		if (!values["cpf-cartao"]) {
			errors["cpf-cartao"] = "campo obrigatorio de ser preenchido";
		} else if (!/^\d{3}\.\d{3}\.\d{3}-\d{2}$/i.test(values["cpf-cartao"])) {
			errors["cpf-cartao"] = "CPF do cartão invalido";
		}

		if (!values["num-parcelas-cartao"]) {
			errors["num-parcelas-cartao"] = "campo obrigatorio de ser preenchido";
		}
	...
	async function handleSubmitCustomForm(values) {
		...
		const [monthExpiration, yearExpiration] =
			values["validade-cartao"].split("/");

		try {
			handleLoadingCheckout({ isLoading: true });
			...
			console.log("finally");
			handleLoadingCheckout({ isLoading: false });
		}
	}
			...
			<div className="sn__checkout-wrapper">
				...
				{productsInCart.length > 0 ? (
					<div className="sn__checkout-finalization">
						<form
							className="sn__checkout-finalization-form"
						...
						>
							<input
								...
								id="bandeira-cartao"
								required
								value={formik.values["bandeira-cartao"]}
							/>
							<input
								type="hidden"
								name="id-bandeira-cartao"
								id="id-bandeira-cartao"
								required
								value={formik.values["id-bandeira-cartao"]}
							/>
							...
							<div className="sn__checkout-finalization-form-step one-step">
								<div className="field">
									...
									{formik.touched.nome && formik.errors.nome ? (
										<span className="field-error-feedback">
											{formik.errors.nome}
										</span>
									) : null}
								</div>
								...
								<div className="field">
									...
									<input
										...
										name="sobrenome"
										id="sobrenome"
										placeholder="ex: Pereira Oliveira"
								...
								<div className="field">
									...
									<input
										...
										name="email"
										id="email"
										placeholder="ex: bernardo@gmail.com"
										autoComplete="email"
										required
							...
							<div className="sn__checkout-finalization-form-step three-step">
								<div className="field">
									...
									<InputMask
										...
										required
										value={formik.values["num-cartao"]}
										onChange={(e) => {
											formik.handleChange(e);
										...
										maskChar=""
									/>
									{formik.touched["num-cartao"] &&
									formik.errors["num-cartao"] ? (
										<span className="field-error-feedback">
											{formik.errors["num-cartao"]}
										</span>
									) : null}
								...
								<div className="field">
									...
									<InputMask
										...
										autoComplete="cc-exp"
										required
										value={formik.values["validade-cartao"]}
										onChange={formik.handleChange}
										onBlur={formik.handleBlur}
										mask="99/9999"
										maskChar=""
									/>
									{formik.touched["validade-cartao"] &&
									formik.errors["validade-cartao"] ? (
										<span className="field-error-feedback">
											{formik.errors["validade-cartao"]}
										</span>
									) : null}
								...
								<div className="field">
									<label htmlFor="cod-seguranca-cartao">
										cod. de segurança
									</label>
									<input
										type="number"
										name="cod-seguranca-cartao"
										id="cod-seguranca-cartao"
										autoComplete="cc-csc"
										required
										value={formik.values["cod-seguranca-cartao"]}
										onChange={formik.handleChange}
										onBlur={formik.handleBlur}
										maxLength="4"
									/>
									{formik.touched["cod-seguranca-cartao"] &&
									formik.errors["cod-seguranca-cartao"] ? (
										<span className="field-error-feedback">
											{formik.errors["cod-seguranca-cartao"]}
										</span>
									) : null}
								...
								<div className="field">
									...
									<input
										...
										required
										value={formik.values["nome-cartao"]}
										onChange={formik.handleChange}
										onBlur={formik.handleBlur}
									/>
									{formik.touched["nome-cartao"] &&
									formik.errors["nome-cartao"] ? (
										<span className="field-error-feedback">
											{formik.errors["nome-cartao"]}
										</span>
									) : null}
								...
								<div className="field">
									...
									<InputMask
										...
										id="cpf-cartao"
										required
										value={formik.values["cpf-cartao"]}
										onChange={(e) => {
											formik.handleChange(e);
											handleChangeCpfCard(e);
										}}
										onBlur={formik.handleBlur}
										mask="999.999.999-99"
										maskChar=""
									/>
									{formik.touched["cpf-cartao"] &&
									formik.errors["cpf-cartao"] ? (
										<span className="field-error-feedback">
											{formik.errors["cpf-cartao"]}
										</span>
									) : null}
								...
								<div className="field">
									...
									<select
										name="num-parcelas-cartao"
										id="num-parcelas-cartao"
										required
										value={formik.values["num-parcelas-cartao"]}
										onChange={formik.handleChange}
										onBlur={formik.handleBlur}
									>
										{parcelas.length > 0 ? (
											parcelas.map((parcela) => (
												<option value={parcela.installments}>
									...
									{formik.touched["num-parcelas-cartao"] &&
									formik.errors["num-parcelas-cartao"] ? (
										<span className="field-error-feedback">
											{formik.errors["num-parcelas-cartao"]}
										</span>
									) : null}
			...
			{productsInCart.length > 0 ? (
				<div className="sn__checkout-resume">
					<div className="sn__checkout-resume-products">
						<h2 className="sn__checkout-resume-products-title">
							resumo do pedido
						</h2>
									...
									<li>
										<div className="sn__checkout-resume-products-list-product">
											<figure className="sn__checkout-resume-products-list-product-image">
												<img src={product.url_images[0]} alt="sneakears" />
											</figure>
